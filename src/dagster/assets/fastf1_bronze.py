...
def bq_bronze_fastf1_sessions(context) -> None:
    """Parse bronze file to bigquery"""
    table_id = "f1toolbox-core.f1toolbox_core.bq_bronze_fastf1_sessions"
    source_uri = "gs://f1toolbox-bronze-bucket/fastf1/bronze_fastf1_sessions.parquet"
    meta = load_parquet_file_to_table(source_uri, table_id)
    context.add_output_metadata({"Rows": MetadataValue.int(meta["num_rows"])})
    context.add_output_metadata({"Columns": MetadataValue.int(meta["num_cols"])})
